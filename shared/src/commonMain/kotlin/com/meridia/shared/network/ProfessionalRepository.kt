...
interface ProfessionalRepository {
    suspend fun profile(): ProfessionalDto
}
...
class ProfessionalRepositoryImpl(
    ...
    override suspend fun profile(): ProfessionalDto = guarded {
        validated(client.get("$base/professional") { bearerAuth(requireToken()) })
    ...
    private suspend fun validated(response: HttpResponse): HttpResponse {
        ...
        Logger.authError("PROFESSIONAL_HTTP_ERROR", "status=${response.status.value}")
        throw BookingException(detailOrNull(body) ?: "Impossibile caricare il profilo. Riprova.")
    }
